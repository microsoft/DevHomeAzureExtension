function Invoke-SignPackage([string]$Path) {
    ...
    $certName = "Microsoft.Windows.DevHomeAzureExtension"
    ...
    if (-not(Test-Path Cert:\LocalMachine\TrustedPeople\$($cert.Thumbprint))) {
        Export-Certificate -Cert $cert -FilePath "$($PSScriptRoot)\Microsoft.Windows.DevHomeAzureExtension.cer" -Type CERT
        Import-Certificate -FilePath "$($PSScriptRoot)\Microsoft.Windows.DevHomeAzureExtension.cer" -CertStoreLocation Cert:\LocalMachine\TrustedPeople    
        Remove-Item -Path "$($PSScriptRoot)\Microsoft.Windows.DevHomeAzureExtension.cer"
...
function Remove-AzureExtensioneCertificates() {
    Get-ChildItem 'Cert:\CurrentUser\My' | Where-Object {$_.FriendlyName -match 'Microsoft.Windows.DevHomeAzureExtension'} | Remove-Item
    Get-ChildItem 'Cert:\LocalMachine\TrustedPeople' | Where-Object {$_.FriendlyName -match 'Microsoft.Windows.DevHomeAzureExtension'} | Remove-Item
}