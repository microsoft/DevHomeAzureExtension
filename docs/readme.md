...
- Run `Build` from AzureExtension's root directory. You can pass in a list of platforms/configurations
- The AzureExtension MSIX will be in your repo under `AppxPackages\x64\debug`

Alternatively

- Open `DevHomeAzureExtension.sln` in Visual Studio, in the `Solutions Configuration` drop-down menu select `Release` or `Debug`, from the `Build` menu choose `Build Solution`.
